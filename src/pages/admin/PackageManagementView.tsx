        ...
        <div className="mb-8 flex justify-between items-center">
          ...
          <button
            onClick={() => handleOpenModal({ 
                name: '', duration: '1-Month', sessions: 0, pricePerSlot: 0, 
                totalPrice: 0, discount: 0, isGenesisEligible: false, isActive: true 
            } as PackageConfig)} // Pre-fill new package with 'Active: true'
...
const PackageModal: React.FC<PackageModalProps> = ({ pkg, onClose, onSave, isLoading }) => {
  ...
  const finalPricePerFloat = formData.totalPrice > 0 && formData.sessions > 0
    ? (formData.totalPrice / formData.sessions).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
    : 'N/A';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.name || formData.sessions <= 0 || formData.pricePerSlot <= 0 || formData.totalPrice <= 0) {
          ...
          <div className={`p-3 rounded-lg border ${formData.isGenesisEligible ? 'bg-teal-50 border-teal-300' : 'bg-red-50 border-red-300'}`}>
            <label className="flex items-center space-x-2">
             ...
             <p className="text-xs mt-1" style={{ color: formData.isGenesisEligible ? 'green' : 'red' }}>
                Required sessions for eligibility: {GENESIS_CONFIG.minSessions}. Current: {formData.sessions}.
            </p>
          </div>
