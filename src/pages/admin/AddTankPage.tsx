...
const AddTankPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 md:p-8 lg:p-10 bg-gradient-to-br from-[#F0F8FB] via-[#E8F4F9] to-[#F5FAFC]">
           {" "}
      <div className="w-full max-w-4xl mx-auto bg-white/95 backdrop-blur-sm rounded-3xl shadow-lg p-6 sm:p-8 md:p-10 border border-cyan-100/50 hover:border-cyan-200/70 transition-colors duration-300">
               {" "}
        <button
          onClick={() => navigate("/admin/tank-management")}
          className="inline-flex items-center mb-6 sm:mb-8 text-sm sm:text-base font-semibold transition-all duration-300 hover:text-cyan-600 hover:translate-x-1 text-slate-700"
        >
                    <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5 mr-2" />          Back to Tank Inventory        {" "}
        </button>
               {" "}
        <header className="mb-8 sm:mb-10 pb-6 sm:pb-8 border-b-2 border-cyan-200/60">
                   {" "}
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-2">
            ...
            <div className="p-2 rounded-lg bg-cyan-100/40 border border-cyan-300/50">
                           {" "}
              {isEditMode ? (
                <Edit className="w-6 h-6 sm:w-8 sm:h-8 text-cyan-700" />
              ) : (
                <PlusCircle className="w-6 h-6 sm:w-8 sm:h-8 text-cyan-700" />
              )}
                         {" "}
            </div>
                       {" "}
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold bg-gradient-to-r from-cyan-800 to-blue-900 bg-clip-text text-transparent">
                            {isEditMode ? `Edit Tank: ${formData.name || id}` : "Add New Floating Tank"}           {" "}
            </h1>
                     {" "}
          </div>
                   {" "}
          <p className="text-base sm:text-lg text-slate-600 sm:ml-11">
                         
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
            ...
            <div className="lg:col-span-1 space-y-4 sm:space-y-6">
              ...
              <div className="flex items-center gap-2 border-b-2 border-cyan-200/60 pb-3">
                                <Ruler className="w-4 h-4 sm:w-5 sm:h-5 text-cyan-700" />               {" "}
                <h2 className="text-xl sm:text-2xl font-bold text-slate-800">Physical Specs</h2>             {" "}
              </div>
                           {" "}
              <FormField
                id="name"
            ...
            <div className="lg:col-span-2 space-y-4 sm:space-y-6">
                           {" "}
              <div className="flex items-center gap-2 border-b-2 border-cyan-200/60 pb-3">
                                <Info className="w-4 h-4 sm:w-5 sm:h-5 text-cyan-700" />               {" "}
                <h2 className="text-xl sm:text-2xl font-bold text-slate-800">Therapy & Details</h2>             {" "}
              </div>
                           {" "}
              <div className="space-y-2">
                               {" "}
                <label className="flex items-center text-sm font-semibold text-slate-800">
                                    <Text className="w-4 h-4 mr-2 text-cyan-700" />                  Floating Therapy
                  Benefits                {" "}
                </label>
                               {" "}
                <textarea
                  ...
                  className="w-full p-3 sm:p-4 border rounded-xl transition-all duration-200 focus:outline-none focus:ring-2 bg-blue-50/40 border-cyan-200/60 text-slate-800 placeholder-slate-400 focus:ring-cyan-400/60 focus:border-cyan-400/80 focus:shadow-lg focus:shadow-cyan-200/20 text-sm sm:text-base"
                />
                                             {" "}
              </div>
                         {" "}
            ...
                                           {" "}
            {isEditMode && <input type="hidden" name="status" value={formData.status} onChange={handleChange} />}       
             {" "}
          </div>
                   {" "}
          <div className="pt-6 sm:pt-8 border-t border-cyan-200/40">
                       {" "}
            <button
              type="submit"
              className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 text-white font-bold rounded-xl shadow-md transition-all duration-300 hover:shadow-lg hover:shadow-cyan-300/40 flex items-center justify-center gap-2 bg-cyan-600 hover:bg-cyan-700 active:scale-95"
            >
                           {" "}
              {isEditMode ? (
                <>
                                      <Edit className="w-4 h-4 sm:w-5 sm:h-5" />                    Save Changes        
                           {" "}
                </>
              ) : (
                <>
                                      <PlusCircle className="w-4 h-4 sm:w-5 sm:h-5" />                    Add New
                  Floating Tank                  {" "}
                </>
              )}
                         {" "}
...
const FormField: React.FC<FormFieldProps> = ({
  ...
  <div className="space-y-2">
    ...
    <label htmlFor={id} className="flex items-center text-sm font-semibold text-slate-800">
            <Icon className="w-4 h-4 mr-2 text-cyan-700" />      {label}   {" "}
    </label>
       {" "}
    <input
      id={id}
