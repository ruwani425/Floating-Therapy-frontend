...
    _id: string;
    name: string; 
    capacity: number; // Added capacity from API response
    length: number; // Added length from API response
    width: number; // Added width from API response
    benefits: string; // Added benefits from API response
    status: 'Ready' | 'Maintenance'; // Added status from API response
...
const DEFAULT_TANK: Tank = { 
    ...
    status: 'Ready', // Assume default is ready
...
        try {
            // The API response type is unknown but expected to be Tank[] based on the request context
            // Use the provided example structure for response data.
...
    onSave: () => void;
    readyTankCount: number; // **Changed to readyTankCount**
}

const DaySettingsSidebar: React.FC<DaySettingsSidebarProps> = ({ isOpen, onClose, dayData, onSave, readyTankCount }) => {
    
...
    // Calculate total sessions for the entire facility using the **ready tank count**
    // MODIFIED LOGIC: session count * ready tank count
...
                openTime, 
                closeTime,
                finalSessionsToSell // SENDING THE CALCULATED COUNT (based on ready tanks)
            );
...
    const [tanks, setTanks] = useState<Tank[]>([]);
    ...
    // **NEW DERIVED STATE: Ready Tank Count**
...
        // Use the readyTankCount for calculations
...
                    // Use the DYNAMIC ready tank count
...
        }
    }, [startDate, endDate, tanks.length]); // Dependency on tanks.length kept for change detection
...
    useEffect(() => {
        // Trigger fetch if tanks were successfully fetched (tanks.length > 0) OR if the initial load didn't find any tanks (tanks.length === 0 but loading is finished)
        // This ensures the calendar loads correctly even with 0 ready tanks (count is 0)
        if (tanks.length > 0) {
             fetchCalendarData(true);
        } else if (loading && tanks.length === 0) {
             // If loading is true and tanks.length is 0, we assume the API call is complete and returned 0 tanks.
             fetchCalendarData(true);
        }
       
    }, [tanks.length, fetchCalendarData, readyTankCount]); // **Added readyTankCount to dependencies**
...
                dayData.hours.open, 
                dayData.hours.close,
                sessionsToSend // SENDING THE CALCULATED COUNT (based on ready tanks)
            );
            
            fetchCalendarData(); 
        } catch (e) {
...
                                            const isLow = availableCount > 0 && availableCount < readyTankCount; // Check against ready tank count
                                            
                                            return (
...
                    readyTankCount={readyTankCount} // **Pass the ready tank count**
                />
            )}
        </div>
    );
